...
class CreateExercisePage extends React.Component {
  ...
  handleSubmit = async (e) => {
    e.preventDefault();
    const { question, type, options, answer, difficulty, testCode, releaseDate } = this.state;

    // Monta o objeto de dados de exercício a ser enviado para o backend
    const exerciseData = {
      question,
      type,
      options: type === 'multiple_choice' ? options.filter(option => option.trim() !== '') : null, // Filtra as opções vazias
    ...
    return (
      <div className="create-exercise-container">
        ...
        <form className="exercise-form" onSubmit={this.handleSubmit}>
          ...
          <button className="submit-button" type="submit">Criar Exercício</button>
        </form>
      </div>
    );
  }
